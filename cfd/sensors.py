...
class SensorGenerator(ABC):
    ...
    def __init__(
        ...
        self.mask: torch.Tensor | None = None # Added mask support
...
class LHS(SensorGenerator):
    ...
    def __call__(self) -> torch.Tensor:
        ...
        device = self.mask.device if self.mask is not None else 'cpu'
        ...
        if self.mask is not None:
            ...
            selected_indices = valid_indices[perm[:self.n_sensors]]
            return selected_indices.int().to(device)
            
        lhs_samples: np.ndarray = self._sampling()
        # absolute positions
        res_tensor = torch.tensor(data=self.resolution, device=device)
        sensor_positions = torch.from_numpy(lhs_samples).to(device) * res_tensor
